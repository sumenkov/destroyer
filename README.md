...
## GUI (только для Linux, на данный момент)
Для людей, которые предпочитают кликать мышкой вместо ввода флагов, доступен десктопный GUI, созданный
с помощью egui/eframe (чистый Rust, никакой зависимости от GTK — только обычные dev-заголовки X11/Wayland, которые у 
большинства Linux-десктопов уже есть).
...
```
(`sudo` необходим по той же причине, что и для CLI — запись на сырое блочное
устройство требует root-прав.)

**What it does:**
- Выводит список блочных устройств (дисков целиком) из `/sys/class/block` (размер, модель,
  rotational/SSD, и смонтировано ли что-то по этому пути в данный момент,
  согласно `/proc/mounts`) или позволяет ввести путь вручную.
- Позволяет настраивать проходы / режим (`fast`/`durable`/`direct`) / размер буфера, так же
  как и CLI.
- Перед записью чего-либо заставляет вас **заново ввести точный путь к устройству**
  и отметить галочкой чекбокс «I understand this is irreversible» — никакого таймера
  обратного отсчета, который можно случайно прокликнуть.
- Запускает очистку в фоновом потоке, чтобы окно оставалось отзывчивым, с
  живыми шкалами прогресса для каждого прохода и общего итога, а также ETA.
- Имеет кнопку **Cancel**: она устанавливает флаг, который цикл записи проверяет между
  чанками размером с буфер (так что она останавливается в пределах примерно одного объема буфера ввода-вывода,
  не мгновенно, но вполне в пределах секунды для типичных размеров буфера).

На данный момент реализована поддержка только для Linux (`src/bin/destroyer_gui.rs` выводит понятное
сообщение и завершает работу на других ОС) — перечисление устройств в macOS и сборка
GUI под macOS будут добавлены позже.
